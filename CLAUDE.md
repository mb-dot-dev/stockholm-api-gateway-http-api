...
- **Producer** (`app/producer.py`) — invoked by API Gateway via `AWS_PROXY` integration on `POST /`. API Gateway validates the Auth0 JWT *before* invoking. The producer then applies a second-layer **IP allowlist** check (`ALLOWED_IPS` env var, CIDR-aware) and, if allowed, enqueues the raw request body to SQS. Returns `202` on success, `403` if the source IP is not allowed, `500` on SQS failure.
...
- Both handlers use **aws-lambda-powertools** (`Logger`, and `Metrics` for the consumer). `POWERTOOLS_SERVICE_NAME` / `POWERTOOLS_LOG_LEVEL` are set per-function in the SAM templates.
